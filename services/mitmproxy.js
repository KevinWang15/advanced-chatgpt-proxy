...
function runServer(account, port) {
...
module.exports = async function startMitmProxyForBrowser() {
    const ports = await findNFreePorts(config.accounts.length);
    for (let i = 0; i < config.accounts.length; i++) {
        mapAccountNameToPort[config.accounts[i].name] = ports[i];
        mapPortToAccountName[ports[i]] = config.accounts[i].name;
        runServer(config.accounts[i], ports[i]);
    }
}
