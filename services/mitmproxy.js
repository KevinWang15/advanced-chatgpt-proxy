...
server.on('connect', (req, clientSocket, head) => {
    ...
    if (req.url === `${INTERCEPT_DOMAIN}:443`) {
        handleCdnOaiStaticComMitm(clientSocket, head);
    } else {
        handleSocks5Tunnel(req, clientSocket, head);
    }
});
...
function buildHttpResponseHeaders(realResp, bodyBuffer) {
    ...
    // // Now override with correct Content-Length after transformation
    ...
    const tlsServer = new tls.Server({key: ephemeralKey, cert: ephemeralCert}, async (tlsClientSocket) => {
        ...
        try {
            ...
            if (responseCache.has(cacheKey)) {
                ...
                // 2) Fetch real site through SOCKS proxy
                const realUrl = `https://${INTERCEPT_DOMAIN}${path}`;

                // Configure fetch to use SOCKS proxy
                const socksAgentOptions = {
                    proxy: {
                        host: SOCKS_HOST,
                        port: SOCKS_PORT,
                        type: 5, // SOCKS5
                ...
                // Use SocksClient to create a SOCKS-proxied connection
                const socksConnection = await SocksClient.createConnection({
                    proxy: {ipaddress: SOCKS_HOST, port: SOCKS_PORT, type: 5},
                    command: 'connect',
                    destination: {host: INTERCEPT_DOMAIN, port: 443},
                });
 ...
 */
async function handleSocks5Tunnel(req, clientSocket, head) {
    ...
    try {
        ...
        attachErrorHandlers(info.socket);
...
function doReplacements(body) {
    const replacements = [
        {
            pattern: /class (\w+){(.+?);static auth0Client=null;/g,
            replacement: 'class $1{$2;static auth0Client=null;static xxxxx=(function(){window.oaiapi=$1;window.inj1=true;})();'
        },
        {
            pattern: /function wB\(e\)\{/g,
            replacement: 'function wB(e){window.inj2=true;window.oairouter=bx().router;'
        },
        {
