...
function startReverseProxy({doWork}) {
    ...
    app.get('/accounts', (req, res) => {
        res.send(config.accounts.map(x => ({name: x.name, labels: x.labels})));
    });
    ...
    app.all('*', async (req, res) => {
        const selectedAccount = getSelectedAccount(req.headers['x-account-name'] || req.cookies['account_name']);
...
function getSelectedAccount(accountName) {
    if (!accountName) {
        ...
        let user = config.accounts.find(account => account.name === accountName);
        if (user) {
            return user;
        } else {
            return config.accounts[0];
...
async function proxyRequest(req, res, targetHost, targetPath, selectedAccount) {
    try {
        ...
        if (!targetPath.includes('static') && !targetPath.includes('/assets/')) {
            ...
            headers['cookie'] = `${selectedAccount.cookie}`;
        }
...
function initializeDegradationCheck() {
    // Run the first check immediately (for all accounts)
    performDegradationChecks();

    // Schedule checks to run every hour (3600000 ms)
    setInterval(performDegradationChecks, 3600000);
}

/**
 * Perform degradation checks for each configured account in sequence.
 * (If you prefer to run them in parallel, you could map to Promise.all().)
 */
async function performDegradationChecks() {
...
`.trimStart();
    ...
    for (const account of config.accounts) {
        ...
        if (lastDegradationResult) {
            ...
            // Merge default label `account_name` with any custom labels in config.
...
function handleConversation(req, res, {doWork, selectedAccount}) {
    ...
    req.on('end', async () => {
        try {
            ...
            try {
                await doWork(task, res, selectedAccount);
            } catch (e) {
                if (!res.headersSent) {
                    res.writeHead(500, {'Content-Type': 'application/json'});
