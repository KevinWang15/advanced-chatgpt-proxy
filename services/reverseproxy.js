...
function proxyRequest(req, res, targetHost, targetPath) {
    ...
    if (options.headers['origin']) {
        options.headers['origin'] = 'https://chatgpt.com';
    }
    ...
    if (!targetPath.includes("static") && !targetPath.includes("/assets/")) {
        options.headers['authorization'] = `Bearer ${config.proxy.authToken}`;
        options.headers['cookie'] = `${config.proxy.cookie}`;
    }

    // Make the HTTPS request
    const proxyReq = targetPath.includes("/stream") ? https.request(options, (proxyRes) => {
        delete proxyRes.headers['content-security-policy'];
        delete proxyRes.headers['set-cookie'];
        proxyRes.headers['access-control-allow-origin'] = config.server.url;

        // Write headers to client
        res.writeHead(proxyRes.statusCode, proxyRes.headers);

        // Pipe the proxy response directly to client response
        proxyRes.pipe(res);
        ...
        proxyRes.on('end', async () => {
            ...
            if (isTextResponse) {
                ...
                if (req.method === "GET") {
                    ...
                    if (targetPathMatch) {
                        try {
                            await addConversationAccess(targetPathMatch[1], req.cookies?.auth_token); // first come, first served; important for advanced voice mode
                        } catch (ignored) {
                        }
                    }
                }
                ...
                if (targetPath.startsWith('/backend-api/conversation/') && !targetPath.endsWith("generate_autocompletions") && !targetPath.endsWith("download")) {
                    ...
                    const hasAccess = targetPath.includes("conversation/init") || targetPath.includes("conversation/voice") || await checkConversationAccess(conversationId, userIdentity);
                    if (!hasAccess) {
