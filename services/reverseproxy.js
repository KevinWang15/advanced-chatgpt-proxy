...
const {
    ...
    addConversationAccess, getInternalAuthenticationToken
} = require('./auth');
...
function startReverseProxy({doWork}) {
    ...
    app.get('/metrics/degradation', async (req, res) => {
        return await handleCheckDegradation(req, res, {doWork});
    })
    ...
    app.all('*', async (req, res) => {
        try {
            ...
            if (
                ...
                return handleConversation(req, res, {doWork});
            }
...
async function proxyRequest(req, res, targetHost, targetPath) {
    try {
        ...
        if (!targetPath.includes('static') && !targetPath.includes('/assets/')) {
            headers['authorization'] = `Bearer ${config.proxy.accessToken}`;
        ...
        if (isStreamRequest) {
            ...
            if (isTextResponse) {
                const serverUrlWithoutProtocol = config.server.url.replace(/^https?:\/\//, '');
                ...
                modifiedContent = modifiedContent.replace(config.proxy.accessToken, '');
...
// Global variable to store the latest degradation check result
let lastDegradationResult = null;
let lastCheckTime = null;
let checkInProgress = false;

// Initialize the periodic check when the server starts
initializeDegradationCheck();

/**
 * Initialize the periodic degradation check that runs every hour
 */
function initializeDegradationCheck() {
    // Run the first check immediately
    performDegradationCheck();

    // Schedule checks to run every hour (3600000 ms)
    setInterval(performDegradationCheck, 3600000);
}

/**
 * Perform the degradation check with retry logic
 */
async function performDegradationCheck() {
    ...
    while (retries <= maxRetries) {
        try {
            console.log(`Performing degradation check (attempt ${retries + 1})`);
            const result = await checkDegradation();

            // Update the global result and timestamp
            lastDegradationResult = result;
            lastCheckTime = Date.now();

            console.log('Degradation check successful');
            checkInProgress = false;
            return;
        } catch (error) {
            retries++;
            console.error(`Degradation check failed (attempt ${retries}):`, error);
...
/**
 * Handle the HTTP request for degradation metrics
 */
async function handleCheckDegradation(req, res) {
    // If we have a valid result, return it immediately
    if (lastDegradationResult) {
        // Format as Prometheus metrics
        const metricsOutput = `# HELP chatgpt_knowledge_cutoff_date The knowledge cutoff date of the model in Unix timestamp
# TYPE chatgpt_knowledge_cutoff_date gauge
chatgpt_knowledge_cutoff_date ${lastDegradationResult.knowledgeCutoffTimestamp}
...
chatgpt_degradation ${lastDegradationResult.degradation}
# HELP chatgpt_last_check_timestamp The timestamp of the last successful check
# TYPE chatgpt_last_check_timestamp gauge
chatgpt_last_check_timestamp ${lastCheckTime}
`;

        // Set Prometheus-compatible content type and send response
        res.setHeader('Content-Type', 'text/plain; version=0.0.4');
        res.status(200).send(metricsOutput);
    } else {
        // If there's no valid result, return a 500 error
        console.error('No valid degradation data available');
        res.status(500).send('No degradation data available');
    }
}
    ...
    try {
        const response = await axios({
            ...
            headers: {
                ...
                'content-type': 'application/json',
                'x-internal-authentication': getInternalAuthenticationToken()
            },
            data: {
                action: 'next',
                messages: [{
                ...
                theAccessToken: process.env.ACCESS_TOKEN,
            },
            responseType: 'stream'
        });
        ...
        const result = await new Promise((resolve, reject) => {
            ...
            // Set a timeout for stream processing as well
            const streamTimeout = setTimeout(() => {
                reject(new Error('Stream processing timeout'));
            }, 9000); // Slightly less than the overall timeout
            ...
            response.data.on('data', (chunk) => {
                ...
                for (const line of lines) {
                    if (line.startsWith('data: ')) {
                        ...
                        // This is an event marker, the next line will be the data
                    }
                }
            });
        ...
        let degradation = 0;
        if (knowledgeCutoffTimestamp >= 1717113600000) { // August 2024 timestamp
            degradation = 0;
        } else if (knowledgeCutoffTimestamp >= 1640822400000) { // January 2022 timestamp
            degradation = 1;
        } else {
            degradation = 2;
        }
        ...
        return {
            ...
            knowledgeCutoffTimestamp: knowledgeCutoffTimestamp / 1000, // Convert to seconds for Prometheus
            degradation,
 ...
 */
function handleConversation(req, res, {doWork}) {
    ...
    req.on('end', async () => {
        try {
            ...
            try {
                await doWork(task, res, req.cookies?.access_token);
            } catch (e) {
                if (!res.headersSent) {
                    res.writeHead(500, {'Content-Type': 'application/json'});
