...
/**
 * Initialize the periodic degradation checks for each account with random intervals.
 */
...
/**
 * Schedule the initial check for an account at a random time between 0 and 10 minutes.
 */
function scheduleInitialCheckForAccount(account) {
    const initialDelay = getRandomMs(0, 10);
    setTimeout(async () => {
...
function scheduleNextCheckForAccount(account) {
    const nextDelay = getRandomMs(20, 30);
    setTimeout(async () => {
        await performDegradationCheckForAccount(account);
        scheduleNextCheckForAccount(account);
    }, nextDelay);
