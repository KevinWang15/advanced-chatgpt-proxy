...
socketIoHttpServer.listen(1236, "127.0.0.1", () => {
    console.log("Server listening on http://127.0.0.1:1236");
});
...
function doWork(task, res, auth_token, {retryCount = 0} = {}) {
    return new Promise((resolve, reject) => {
        ...
        ackTimeout = setTimeout(() => {
            ...
            if (retryCount < 3) {
                doWork(task, res, auth_token, {retryCount: retryCount + 1}).then(resolve).catch(reject);
            } else {
        ...
        workers[workerId].authToken = auth_token;
...
dynamicNsp.on("connection", (socket) => {
    ...
    socket.on("disconnect", () => {
        ...
        if (workers[workerId] && workers[workerId].responseWriter && !workers[workerId].responseWriter.headersSent) {
            ...
            workers[workerId].responseWriter.end(JSON.stringify({error: 'Worker disconnected unexpectedly'}));
        }
        delete workers[workerId];
    });
        ...
        if (isDone) {
            ...
            if (text.indexOf('"conversation_id": "') >= 0) {
                try {
                    ...
                    addConversationAccess(conversationId, workers[workerId].authToken)
