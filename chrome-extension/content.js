...
// Add this function to handle heartbeats
function startHeartbeat() {
    ...
    heartbeatInterval = setInterval(() => {
        ...
        // Check if we haven't received a heartbeat response in more than 12 seconds
        if (Date.now() - lastHeartbeatResponse > 12000) {
            console.warn('No heartbeat response in 12+ seconds, assuming disconnected');
            workerStatus.connected = false;
            workerStatus.lastAction = 'Heartbeat timeout';
            updateStatusOverlay();
...
async function sendHeartbeat() {
    ...
    try {
        const response = await fetch(`${serverUrl}/worker-heartbeat`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (data.error || !data.active) {
            console.warn('Worker no longer registered on server');
            workerId = null;
            workerStatus.connected = false;
            workerStatus.lastAction = 'Unregistered by server';
            updateStatusOverlay();
        ...
        // Don't reset workerId here, as the server might just be temporarily unavailable
    }
}
